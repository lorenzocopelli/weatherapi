...
class ForecastWeather
{
    ...
    ForecastWeather(Map<String, dynamic> jsonData)
    {
        ...
        jsonForecastDays?.forEach((jsonForecastDay)
        {
            ...
            DayConditionData dayCondition = DayConditionData(
                jsonDayCondition?['icon']
            );

            DayData day = DayData(
                jsonDay?['maxtemp_c'], jsonDay?['mintemp_c'],
                jsonDay?['totalsnow_cm'], jsonDay?['daily_chance_of_rain'],
                dayCondition
            );
            ...
            AstroData astro = AstroData(
                jsonDayAstro?['sunrise'], jsonDayAstro?['sunset'],
                jsonDayAstro?['is_moon_up'], jsonDayAstro?['is_sun_up']
            );
            ...
            jsonForecastHour?.forEach((jsonHour)
            {
                ...
                DayConditionData hourCondition = DayConditionData(
                    jsonHourCondition?['icon']
                );

                HourData hour = HourData(
                    jsonHour['time'], jsonHour['temp_c'], hourCondition
                );
            ...
            ForecastDayData forecast = ForecastDayData(
                jsonForecastDay?['date'], day, astro, hours
            );
        ...
        jsonAlertList?.forEach((jsonAlert)
        {
            AlertData alert = AlertData(
                jsonAlert['headline'], jsonAlert['msgtype'],
                jsonAlert['severity'], jsonAlert['urgency'],
                jsonAlert['areas'], jsonAlert['category'],
                jsonAlert['certainty'], jsonAlert['event'],
                jsonAlert['note'], jsonAlert['effective'],
                jsonAlert['expires'], jsonAlert['desc'],
